...
The target used in the experiments is mounted on two separate target ladders. The production ladder hosts the target for the production, while optics ladders contain the target that is used for experiment calibrations. Both the two target ladders are attached to the motion system and can be controlled separately in the counting house. During the experiment, the target can be placed on the central beamline as needed. 

The production target ladder contains 10 208Pb target, one $1\%$ Carbon target,  carbon hole target used for alignment, and Ca target used CRex experiment. 
...
\begin{itemize}
    ...
    \item table of targets in different ladders
    \item cooling
