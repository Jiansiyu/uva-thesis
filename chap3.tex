...
Because of the size limitations, the minimum measurement angle of HRS is $12.5^\circ$. but the PRex-II and CRex experiments is need to do the measurement at angle $5$. The septum magnet is place between the collimator and Q1 to pre-bend the scattered beam to match the minimum angle requirement of the HRS. 

The septum magnet is a non-superconducting magnet. As showed in figure[tobe added], the septum contains coils on the left and right beamlines. The scattered electrons will be bend to the HRS by the up-down magnetic field while the unscattered beam will goes through the center beamline to the beam dump. 

The septum magnet uses 3-col configurations, which can rescue the vertical bore dimension and reduce the could current as a result will reduce the thermal power relative to the required field integral. 
