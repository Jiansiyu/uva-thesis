...
The spectrometer central angle $\theta_0$ is defined as the angle between the beam down direction and the central spectrometer axis. As described in equation \ref{eq:chapter4_scattered_angle_from_central_angle}, the spectrometer's central angle plays a basic input in calculating the particle scattered angle. The laser survey gives Spectrometer's central angle. $\theta_0$ can be measured using a survey measuring the angle between two imaginary lines. The first line is along the ideal spectrometer axis and the second line is along the ideal electron beam direction. 

However, multiple transformations required to relate the ideal spectrometer angle to the actual spectrometer angle could increase the uncertainty of $\theta_0$ measurement to be as much as 0.06 or $~1\%$, which could lead to $2\%$ uncertainty in Q2 measurement in the case of PREX. However, PREX used a new technique in addition to a survey. This technique (called pointing measurement) makes use of the idea of differential nuclear recoil for elastic electron scattering off target nuclei of different masses. The accuracy achieved from pointing measurement is usually better than a survey since pointing measurement does not require multiple transformations as in the case of a survey. Moreover, the accuracy of pointing measurement can be greatly enhanced by considering differential nuclear recoil off different target nuclei that are in the same target such as water (H2O). The benefit of this method is that all uncertainties from estimating energy losses that occurred before and after the scattering are suppressed. Also, this method eliminates the problem of possible beam position shifting during the run. The main uncertainty that contributed to the uncertainty of a pointing measurement is from the ability to measure the energy difference between two different peaks, which, in the case of PREX, had an accuracy at the level of ~30 keV.  

Consider an electron scattered on target with mass $M$, the scattered electron momentum can be written as:

\begin{equation}
    E' = \frac{E - E_{loss1}}{1 + \frac{2(E-E_{loss1})\sin^2{\frac{\theta}{2}}}{M}} - E_{loss2}
\end{equation}

Here in the equation, $E$ is the initial energy of the electrons, $\theta$ is the scattered angle, $E_{loss1}$ is the energy loss that occurred before the scattering, and $E_{loss2}$ is the energy loss occurred after the scattering. 

The water target contains $^{1}H$ and $^{16}O$, which made it possible to scatter two targets with exactly the same beam energy, scattered angle, and even energy loss. The energy loss would be canceled out, and increase the measurement accuracy. The scattered electron energy difference between oxygen and hydrogen can be given by:

\begin{equation}
    \Delta{E'} = E'_O - E'_H = E (\frac{1}{1 + \frac{2E\sin^2{\frac{\theta}{2}}}{M_O}} - \frac{1}{1 + \frac{2E\sin^2{\frac{\theta}{2}}}{M_H}}) + corrections
\end{equation}

Where in the equation, the correction is negligible. Both the beam energy and the mass of the  oxygen and hydrogen are measured with high accuracy. and with the scattered momentum reconstructed from the vertical drift chamber. put in the above equation, the scattered angle can be measured. 


CRex experiment uses exactly the same configuration of the spectrometer, but the beam energy is at 2.18GeV, which gives a much clean energy spectrum. In the data analysis, the CRex data instead of PRex data are used in order to get higher accuracy measurements. 

The pointing measurement was done with a sieve in, and the event passes the central sieve hole is used in the pointing calculation with beam position along the center of the Hall A Coordinate System. This will eliminate the errors caused by the HRS optics reconstruction errors. \ref{fig:pointing_h2o_spectrum} is an example of the waterfall target spectrum for CRex RHRS run 21737. The first peak on the left and the first peak on the right is the ground state energy of the $^{1}H$ and $^{16}O$ respectively. The energy difference is $\Delta E = 16.126 \pm 0.024 MeV$. In order to get a highly accurate central angle, the pointing measure is carried out with different central momentum. With different central momentum, the scattered electrons will end in different VDC area. Each central momentum run will able to provide independent energy difference between hydrogen and oxygen spectrum energy difference.With accurate optics reconstruction, different central momentum gives different energy differences between the hydrogen and oxygen targets, but it should give the same spectrum angles. This will help us verify the calibration accuracy. To avoid drift during the experiment, the pointing measurement also did three times, one at an early stage of the experiment, one during the experiment, and another verification pointing measurement after the experiment. The final pointing measurement are the average of those runs. In table list all the runs used in the pointing measurement. 


In the pointing, although the beam position is set to (0,0), because of the control errors, it can not really reach $(0,0)$. To avoid the errors caused by the beam position, the final angle is corrected by $\frac{X_{bpm}}{Z_0}$, where $Z_0$ is the distance from the target center to the sieve slide along the spectrum central ray [check?], and $X_{bpm}$. is the beam position on $X$ direction in Hall A coordinate system.  
